...
@register_task("translation_thor")
class TranslationThorTask(TranslationMultiSimpleEpochTask):
    ...
    def __init__(self, cfg, langs, dicts, training):
        super().__init__(cfg, langs, dicts, training)
        self.cfg = cfg 
...
@register_task("translation_thor_single", dataclass=TranslationThorConfig)
class TranslationThorTask_Single(TranslationTask):
    """
    Translation task for Switch Transformer models.
    Args:
        src_dict (~fairseq.data.Dictionary): dictionary for the source language
