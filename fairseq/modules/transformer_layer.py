...
from fairseq.modules import LayerNorm, MultiheadAttention, MultiheadAttentionIN
...
class TransformerEncoderLayerBaseIN(nn.Module):
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def __init__(
        self, cfg, active_proj, active_ffn, return_fc=False):
        super().__init__()
        self.cfg = cfg
        self.return_fc = return_fc
        self.embed_dim = cfg.encoder.embed_dim
    ...
    def build_fc1(self, input_dim, output_dim, q_noise, qn_block_size, expert_num, active_ffn):
        num = expert_num if active_ffn[0] else 1
        return Switcher(
            nn.ModuleList([
                quant_noise(
                nn.Linear(input_dim, output_dim), p=q_noise, block_size=qn_block_size
    ...
    def build_fc2(self, input_dim, output_dim, q_noise, qn_block_size, expert_num, active_ffn):
        num = expert_num if active_ffn[1] else 1
        return Switcher(
                nn.ModuleList([
                quant_noise(
                nn.Linear(input_dim, output_dim), p=q_noise, block_size=qn_block_size
...
class TransformerDecoderLayerBaseIN(nn.Module):
    """Decoder layer block.
