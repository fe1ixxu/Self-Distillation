...
class Top2Gating(nn.Module):
    ...
    def forward(self, x, importance = None):
        ...
        *_, b, group_size, dim = x.shape
        ...
        mask_1 = F.one_hot(index_1, num_gates).type(precision)

        density_1_proxy = raw_gates

        if importance is not None:
            equals_one_mask = (importance == 1.).type(precision)
        ...
        mask_2 = F.one_hot(index_2, num_gates).type(precision)

        if importance is not None:
            greater_zero_mask = (importance > 0.).type(precision)
        ...
        if policy == "all":
            ...
            mask_2 *= (gate_2 > threshold).type(precision)
        elif policy == "random":
            probs = torch.zeros_like(gate_2).uniform_(0., 1.)
            mask_2 *= (probs < (gate_2 / max(threshold, self.eps))).unsqueeze(-1).type(precision)
        else:
        ...
        mask_1 *= (position_in_expert_1 < expert_capacity_f).type(precision)
        # [batch, experts]
        ...
        mask_2 *= (position_in_expert_2 < expert_capacity_f).type(precision)
...
class MoE(nn.Module):
    def __init__(self,
        ...
        self.output_dim = output_dim if output_dim else dim
    ...
    def forward(self, inputs, **kwargs):
        ...
        orig_shape = list(expert_inputs.shape)
        expert_inputs = expert_inputs.reshape(e, -1, d)
        expert_outputs = self.experts(expert_inputs)
        orig_shape[-1] = self.output_dim
...
class HeirarchicalMoE(nn.Module):
    ...
    def forward(self, inputs, **kwargs):
        ...
        importance = 0.5 * ((importance > 0.5) + (importance > 0.))
